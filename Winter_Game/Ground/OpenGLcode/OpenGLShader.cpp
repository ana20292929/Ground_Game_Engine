...
namespace Ground
{
	OpenGLShader::OpenGLShader(const std::string& vertexFile, const std::string& fragmentFile)
	{
        ...
        if (!vertexSource.is_open())
            GROUND_LOG("ERROR: Failed to open vertex shader file!");
        ...
        if (!fragmentSource.is_open())
            GROUND_LOG("ERROR: Failed to open fragment shader file!");
        ...
        if (!success)
        {
            ...
            GROUND_LOG("ERROR: Failed to open fragement shader file!");
        }
    ...
    void OpenGLShader::ProvideFloatValues(const std::string& valueName, const std::vector<float>& vals)
    {
        ...
        case 4:
            ...
            break;
        default:
            GROUND_LOG("ERROR: Shader values must be vectors of size 1-4!!");
        }
    }
